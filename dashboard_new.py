...
# ============================================================================
# DATA FUNCTIONS (REVERTED TO YOUR ORIGINAL)
# ============================================================================
...
# ============================================================================
# CALCULATION LOGIC (REVERTED TO YOUR ORIGINAL)
# ============================================================================
...
def define_unique_fp_detections(classified_pred_df, scenario_details_df, iou_threshold=0.8, return_clusters=False):
    ...
    if fp_df.empty:
        return (0, fp_df, {}) if return_clusters else 0
    ...
    def vectorized_iou(boxes1, boxes2):
        b1_exp, b2_exp = boxes1[:, np.newaxis, :], boxes2[np.newaxis, :, :]
        xa1, ya1 = np.maximum(b1_exp[:, :, 0], b2_exp[:, :, 0]), np.maximum(b1_exp[:, :, 1], b2_exp[:, :, 1])
        xa2, ya2 = np.minimum(b1_exp[:, :, 2], b2_exp[:, :, 2]), np.minimum(b1_exp[:, :, 3], b2_exp[:, :, 3])
        inter = np.maximum(0.0, xa2 - xa1) * np.maximum(0.0, ya2 - ya1)
        area1, area2 = (boxes1[:, 2] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 1]), (boxes2[:, 2] - boxes2[:, 0]) * (
                    boxes2[:, 3] - boxes2[:, 1])
    ...
    for _, group in grouped:
        ...
        indices, boxes = group['detection_idx'].values, group[['xmin', 'ymin', 'xmax', 'ymax']].values
        iou_matrix = vectorized_iou(boxes, boxes)
        i_idx, j_idx = np.where(np.triu(iou_matrix >= iou_threshold, k=1))
        for i, j in zip(i_idx, j_idx): union(indices[i], indices[j])
    ...
    unique_fp_count = len(clusters)
    return (unique_fp_count, fp_df, clusters) if return_clusters else unique_fp_count
...
# ============================================================================
# APP UI (REVERTED TO YOUR ORIGINAL)
# ============================================================================
client = get_bigquery_client()
...
st.sidebar.markdown("---")
st.sidebar.header("🧪 Comparison Rows")
with st.sidebar.expander("➕ Add Special Scenario to Table", expanded=False):
    if selected_models:
        run_m = st.selectbox("Model", selected_models)
        run_app = st.multiselect("Run Appearance", sorted(ann_df['Appearance_Level'].unique().tolist()),
                                 placeholder="Global Selection")
        run_occ = st.multiselect("Run Occlusion", sorted(ann_df['Occlusion_Level'].unique().tolist()),
                                 placeholder="Global Selection")
        run_dist = st.multiselect("Run Distance", sorted(ann_df['Distance'].unique().tolist()),
                                  placeholder="Global Selection")
        run_mot = st.multiselect("Run Motion", sorted(ann_df['Motion'].unique().tolist()),
                                  placeholder="Global Selection")
        run_uni = st.multiselect("Run Uniform", sorted(ann_df['Uniform'].unique().tolist()),
                                 placeholder="Global Selection")
        run_lig = st.multiselect("Run Lighting", get_filter_options(client, "Lighting_Condition", SCENARIO_DETAILS),
                                 placeholder="Global Selection")
        run_cam = st.multiselect("Run Camera", get_filter_options(client, "Camera_Movement", SCENARIO_DETAILS),
                                 placeholder="Global Selection")


        if st.button("Add Row"):
            # We create a list of which filters the user actually touched
            custom_cols = []
            if run_app: custom_cols.append("Appearance")
            if run_occ: custom_cols.append("Occlusion")
            if run_dist: custom_cols.append("Distance")
            if run_mot: custom_cols.append("Motion")
            if run_uni: custom_cols.append("Uniform")
            if run_lig: custom_cols.append("Lighting")
            if run_cam: custom_cols.append("Camera")


            st.session_state.evaluation_runs.append(
                {"model": run_m,
                 "app": run_app if run_app else f_app,
                 "occ": run_occ if run_occ else f_occ,
                 "dist": run_dist if run_dist else f_dist,
                 "mot": run_mot if run_mot else f_mot,
                 "uni": run_uni if run_uni else f_uni,
                 "lig": run_lig if run_lig else s_light,
                 "cam": run_cam if run_cam else s_cam,
                 "changed_filters": custom_cols
...
if st.session_state.evaluation_runs:
    if st.sidebar.button("🗑️ Clear Comparison Rows", use_container_width=True):
        st.session_state.evaluation_runs = []
        st.rerun()
...
if selected_models and not ann_df.empty:
    with st.spinner("Processing..."):
        ...
        if not full_df.empty:
            ...
            # Identify which columns actually have custom data across ALL runs
            active_custom_cols = set()
            for run in st.session_state.evaluation_runs:
                active_custom_cols.update(run['changed_filters'])
                for col in run['changed_filters']:
                    active_custom_cols.add(col)

            # Convert set to a sorted list for consistent table headers
            active_custom_cols = sorted(list(active_custom_cols))

            comp_list = []
            sc_details_df = client.query(f"SELECT scenario,"
                                         f" cluster,"
                                         f" Lighting_Condition,"
                                         f" Camera_Movement FROM `{SCENARIO_DETAILS}`").to_dataframe()

            # Logic to handle both global and custom rows
            for m in selected_models:
                ...
                track_stats = calculate_track_metrics(tp_sub, subset[subset['effective_type'] == 'FP'], filtered_ann,
                                                      total_clusters, sc_details_df)
                row.update(track_stats)
                row.update({'Model': m})
                row.update({col: "Global" for col in active_custom_cols})
                comp_list.append(row)

                # Custom Rows for this specific model
                for run in [r for r in st.session_state.evaluation_runs if r['model'] == m]:
                    # A. Find scenarios that match the Lighting and Camera selection
                    valid_scenarios_for_run = sc_details_df[
                        (sc_details_df['Lighting_Condition'].isin(run['lig'])) &
                        (sc_details_df['Camera_Movement'].isin(run['cam']))
                        ]['scenario'].tolist()

                    # B. Now filter the ann_df using that scenario list + the object attributes
                    c_ann = ann_df[
                        (ann_df['scenario'].isin(relevant_sc_list)) &
                        (ann_df['Appearance_Level'].isin(run['app'])) &
                        (ann_df['Occlusion_Level'].isin(run['occ']))  &
                        (ann_df['Distance'].isin(run['dist'])) &
                        (ann_df['Motion'].isin(run['occ'])) &
                        (ann_df['Uniform'].isin(run['dist']))
                    ]
                    c_keys = set(c_ann['gt_key'].tolist());
                    c_total_gt = len(c_keys) if len(c_keys) > 0 else 1
                    c_metrics = calculate_advanced_metrics(m_df, c_keys, c_total_gt, d_h, fpiou_h, amount_of_frames)
                    c_row = c_metrics.iloc[(c_metrics['threshold'] - t).abs().idxmin()].to_dict()
                    c_tp = subset[(subset['effective_type'] == 'TP') & (subset['gt_key'].isin(c_keys))]
                    c_track = calculate_track_metrics(c_tp, subset[subset['effective_type'] == 'FP'], c_ann,
                                                      total_clusters, sc_details_df)
                    c_row.update(c_track)
                    c_row.update(
                        {'Model': f"↳ {m}"})
                    mapping = {
                        "Appearance": "app", "Occlusion": "occ", "Distance": "dist",
                        "Motion": "mot", "Uniform": "uni", "Lighting": "lig", "Camera": "cam"
                    }

                    for f in active_custom_cols:
                        if f in run['changed_filters']:
                            # If the user specifically changed this filter for this row, show the selection
                            c_row[f] = ", ".join(run[mapping[f]])
                        else:
                            # If the user left this filter as 'Global' for this specific row
                            c_row[f] = "Global"

                    comp_list.append(c_row)

            comp_df = pd.DataFrame(comp_list)
            best_m = comp_df.loc[comp_df['Detection f1'].idxmax(), 'Model']

            # Restored Styling and Formats
            standard_cols = ['Model']
            metric_cols = [
                'Detection Recall', 'False Alarm Rate', 'Detection Precision',
                'Detection f1', 'Track Recall Obs', 'Track Recall Alerts',
                'Track FAR', 'Amount of Track FP', 'Avg TPs Confidence'
            ]

            # Combine them: [Model] + [Only the filters you used] + [Metrics]
            final_column_order = standard_cols + active_custom_cols + metric_cols

            st.dataframe(comp_df[final_column_order].style.apply(
            ...
            global_only = [x for x in comp_list if "↳" not in x['Model']]
            for i, m in enumerate(selected_models):
                data = next(item for item in global_only if item['Model'] == m)
                curr_t = st.session_state.selected_thresholds.get(m, 0.5)
