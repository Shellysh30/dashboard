...
#     # This will store our custom filter rows for the comparison table
    ...
    if selected_models:
        ...
        if st.button("Add Row"):
            st.session_state.evaluation_runs.append(
                {"model": run_m, "app": run_app if run_app else f_app, "occ": run_occ if run_occ else f_occ})
            st.rerun()
    ...
    with st.spinner("Processing..."):
        ...
        if not full_df.empty:
            ...
            for m in selected_models:
                ...
                row.update(track_stats);
                row.update({'Model': m, 'Appearance': 'All', 'Occlusion': 'All'})
                comp_list.append(row)

                # Custom Rows for this specific model
                for run in [r for r in st.session_state.evaluation_runs if r['model'] == m]:
                    c_ann = ann_df[
                        (ann_df['scenario'].isin(relevant_sc_list)) & (ann_df['Appearance_Level'].isin(run['app'])) & (
                    ...
                    c_row.update(
                        {'Model': f"↳ {m}", 'Appearance': ", ".join(run['app']), 'Occlusion': ", ".join(run['occ'])})
            ...
            st.dataframe(comp_df[
                ['Model', 'Appearance', 'Occlusion', 'Detection Recall', 'False Alarm Rate', 'Detection Precision',
                 'Detection f1', 'Track Recall Obs', 'Track Recall Alerts', 'Track FAR', 'Amount of Track FP',
                 'Avg TPs Confidence', 'TP', 'FP', 'FN']].style.apply(
                lambda r: ['color: #FF1493; font-weight: bold' if r['Model'] == best_m else '' for _ in r], axis=1).map(
                lambda x: color_metrics(x, 'high'),
                subset=['Detection Recall', 'Detection Precision', 'Detection f1']).map(
